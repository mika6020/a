...
function getformattedLocalTime(area)
  if area == nil then
    return "🔱 زمانی برای اسن مکان ثبت نشده است . 🔱"
  end

  lat,lng,acc = get_latlong(area)
  if lat == nil and lng == nil then
    return '🔱 بنظر میرسد که در ("'..area..'") مفهوم زمان را ندارد 🔱'
  end
  local localTime, timeZoneId = get_time(lat,lng)

  return "🔱 زمان ثبت شده در ("..timeZoneId.." ) تاریخ و ساعت (".. os.date(dateFormat,localTime)") است .🔱"
end
